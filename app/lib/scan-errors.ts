...
export function translateScanError(
  ...
  if (msg.includes('replicate timeout') || (msg.includes('timeout') && msg.includes('replicate'))) {
    return {
      title: 'Stobi думает дольше обычного',
  ...
  if (msg.includes('nsfw') || msg.includes('moderation')) {
    return {
      title: 'Это не камень',
      message: 'В кадре что-то другое — Stobi не разрешил сохранить это фото.',
      tips: [
  ...
  if (kind === 'find-anywhere') {
    return {
      title: 'Stobi не узнал этого камня',
      message: 'Возможно, его раскрасил кто-то сам и пока не знает про Stobi. Или фото получилось не очень — попробуй ещё.',
      tips: [
        'Если уверен что камень из Stobi — попробуй другой ракурс или поднеси ближе',
        'Проверь освещение — рисунок должен быть чётко виден',
        'Если камень "ничей" — попробуй найти автора в соцсетях по рисунку',
        'Можешь спрятать его сам и стать частью игры 💜',
      ],
    };
  }
