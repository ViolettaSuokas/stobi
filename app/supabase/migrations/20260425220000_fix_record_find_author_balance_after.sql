-- Fix: record_find_v2 падал на balance_events insert когда начислял
-- автору (v_author_reward). Передавал balance_after=NULL, а колонка
-- NOT NULL.
--
-- Симптом: AI находил match → проходила embedding-проверка → доходило
-- до начисления автору → INSERT INTO balance_events с null →
-- "null value in column balance_after violates not-null constraint" →
-- finder получал rpc_error и НЕ получал свою награду.
--
-- Fix: capture balance автора в переменную через RETURNING, как для finder'а.
...
begin
  ...
  if v_status = 'verified' then
    ...
    -- FIX: было `balance_after=NULL` (потому что не делал RETURNING), а
    -- колонка NOT NULL → весь find падал. Теперь захватываем new balance.
