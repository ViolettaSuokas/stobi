-- Adaptive learning: append proof embedding to stone.alt_embeddings after
-- any successful verified find (not только после author_approve).
--
-- Изначально adaptive learning срабатывал только в author_approve_pending_find.
-- Теперь — и при автоматическом verified (AI-match / AI+GPS) тоже, чтобы
-- embedding-база камня пополнялась с каждой находки → следующие сканы
-- становятся точнее (разные ракурсы / освещение).
...
begin
  ...
  if v_status = 'verified' then
    ...
    -- Adaptive learning: добавляем embedding найденного фото в
    -- stone.alt_embeddings[] (FIFO max 10). Только если reason это
    -- AI-match — для GPS-only legacy find embedding не надо (он
    -- может быть мусорным, камень без reference).
