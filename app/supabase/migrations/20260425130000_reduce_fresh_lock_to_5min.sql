-- record_find_v2: УБРАТЬ fresh-lock полностью.
--
-- Был interval '1 hour'. Реальный UX-кейс: спрятал в парке, мама прошла
-- через 5 минут с ребёнком, нашла, попыталась отметить — "недоступно".
-- Анти-фрод и без этого солидный:
--   - daily_find_limit (≤10 находок/юзер/день)
--   - author_daily_limit (≤20 находок чужих камней этого автора/день)
--   - global_author_limit (≤100 находок камней этого автора всего)
--   - cannot_find_own_stone (свой нельзя)
--   - AI similarity ≥0.70 (нужно реально показать камень)
--   - already_found (один и тот же юзер не может отметить дважды)
--
-- Тело функции 1:1 повторяет миграцию 20260422160000_harden_rpcs_and_constraints.sql,
-- удаляем только блок "stone_too_fresh".
...
begin
  ...
  -- ── УДАЛЕНО: fresh-lock check (был interval '1 hour'). Анти-фрод
  --    держится на daily_find_limit + author_daily_limit + cannot_find_own.
