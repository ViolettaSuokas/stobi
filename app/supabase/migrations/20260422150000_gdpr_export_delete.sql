-- GDPR Article 15 (right of access). App Store Review guideline 5.1.1(v)
-- requires apps with accounts to expose a data-export mechanism.
--
-- Deletion is already handled by existing `delete_user()` RPC (deletes
-- from auth.users which cascades to profiles/balance_events/stones/finds
-- via the FK chain). This migration only adds the missing export.
--
--   gdpr_export_my_data() → returns a single JSONB blob with every row
--                           tied to the current user. Client writes it
--                           to disk via expo-file-system and shares.
...
begin
  ...
  select jsonb_build_object(
    ...
    'finds', coalesce((select jsonb_agg(row_to_json(f) order by f.created_at)
