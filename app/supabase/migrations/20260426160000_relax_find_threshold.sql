-- Relax AI similarity threshold in record_find_v2 — task: trust AI more,
-- remove "pending" state for matches > 0.70.
--
-- Раньше:
--   sim > 0.82          → verified (auto)
--   0.70 ≤ sim < 0.82
--     + GPS within 30m  → verified (auto)
--     - no GPS          → pending (author approval)
--   sim < 0.70          → rejected
--
-- Теперь (по запросу юзера "ты же хорошо определяешь чей камень"):
--   sim ≥ 0.70 → verified (auto, regardless of GPS)
--   sim < 0.70 → rejected
--
-- Pending-state не достижим из record_find_v2 → "Одобрить находки" UI
-- не нужен. Триггер notify_author_on_pending_find останется (не дёргается),
-- find_proofs всё равно пишутся для аудита (status='verified' or 'rejected').

create or replace function public.record_find_v2(p_stone_id uuid, p_photo_url text, p_embedding vector, p_proof_lat double precision DEFAULT NULL::double precision, p_proof_lng double precision DEFAULT NULL::double precision, p_nsfw_labels jsonb DEFAULT NULL::jsonb)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  -- ★ NEW THRESHOLDS — trust AI, no more pending ★
  if v_has_embedding then
    if v_best_sim >= 0.70 then
      v_status := 'verified';
      v_reason := case when v_gps_ok then 'ai_match_plus_gps' else 'ai_match' end;
    else
      v_status := 'rejected';
  ...
  if v_status = 'verified' then
    ...
    insert into balance_events (user_id, amount, reason, ref_id, balance_after)
      values (v_stone.author_id, v_author_reward, 'author_bonus', p_stone_id::text, v_author_balance);
